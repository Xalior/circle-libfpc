...
//
// It returns. The host kernel brings the board up and calls the Pascal
// entry point; halting hands control back to it. fpc_do_exit returns to
// PASCALMAIN, PASCALMAIN returns to the host kernel's call site, and the
// host kernel decides what happens next, since the host kernel owns the
// cores and every device on the board.
//
