//
// kernel.cpp — circle-libfpc M3: the Pascal program measures time.
//
// The host kernel is M1's, unchanged in everything it does: bring the board up
// on core 0, arm the runtime on every core, activate the shim's core split,
// then let the application core go.
//
// NOTHING HERE ARRANGES A CLOCK FOR THE PASCAL PROGRAM, and that is the point.
// Elapsed time on this board is the Arm generic timer's free-running counter,
// which is a processor system register: the firmware set its frequency before
// any of this ran, and it has been counting since. The application core reads
...
static const char From[] = "libfpc-m3";
...
TShutdownMode CKernel::Run(void)
{
    m_Logger.Write(From, LogNotice,
                   "circle-libfpc M3: hardware core 0, application core 1");
