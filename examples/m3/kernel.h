//
// kernel.h — circle-libfpc M3: a Pascal program that measures time.
//
// The core layout is the product's. Core 0 is the hardware core and owns every
// device on the board, the serial console included. Core 1 runs the Pascal
// program, and that core is the whole of the computer the Pascal program can
// see: it owns no device, drives no UART, and reaches nothing across a core
// boundary itself. Cores 2 and 3 are parked — this example draws nothing, so
// there is no presentation worker to run.
//
#ifndef _kernel_h
#define _kernel_h
