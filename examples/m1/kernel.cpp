//
// kernel.cpp — circle-libfpc M1: the Pascal program has a heap.
//
// The host kernel is M2's, unchanged in everything it does: bring the board up
// on core 0, arm the runtime on every core, activate the shim's core split,
// then let the application core go. Nothing here installs the memory manager
// and nothing here allocates on the Pascal program's behalf. The Free Pascal
// runtime installs it as the first act of the program, from the other side of
// PASCALMAIN, onto the allocator Circle already had running.
//
...
static const char From[] = "libfpc-m1";
...
TShutdownMode CKernel::Run(void)
{
    m_Logger.Write(From, LogNotice,
                   "circle-libfpc M1: hardware core 0, application core 1");
